---
title: Bruxos Modernos
wiki_slug: bruxos-modernos
schema_version: 2
status: active
genres: []
infobox:
  image: 03_Resources/source_material/ArtistPortraits/bruxos_modernos.jpg
  caption: Bruxos Modernos
  background: solo_singer
audio_profile:
  tempo_avg: 132.4
  energy_avg: 0.794
  valence_avg: 0.805
  danceability_avg: 0.7
  acousticness_avg: 0.268
  instrumentalness_avg: 0.004
  loudness_avg: -6.2
  speechiness_avg: 0.059
  liveness_avg: 0.146
  track_count: 9
  computed_at: '2025-12-23T08:16:06.157841'
similar_artists: []
musical_connections:
  collaborators: []
  influenced: []
  mentors: []
spotify_data:
  followers: 91
  id: 4BqrcNbCVneZqkMWpMzGzP
  popularity: 0
  url: 'https://open.spotify.com/artist/4BqrcNbCVneZqkMWpMzGzP'
discography_enriched: true
top_tracks:
  - Alguém Te Vê (Alguém Te Vê)
  - Essa Mentira (Não Cabe em Você) (Alguém Te Vê)
  - Bonita da Balada (Alguém Te Vê)
  - Linda Blair (Alguém Te Vê)
  - Cobras no Paraíso (Alguém Te Vê)
  - Makina Loka (Makina Loka)
  - Sister Mary (Makina Loka)
  - Ninguém é Dorian (Makina Loka)
  - Gagá (Makina Loka)
external_urls:
  spotify: 'https://open.spotify.com/artist/4BqrcNbCVneZqkMWpMzGzP'
  wikipedia: ''
image_path: 03_Resources/source_material/ArtistPortraits/bruxos_modernos.jpg
biography_enhanced_at: '2025-12-23T02:05:59.668536'
primary_source: perplexity
enhancement_provider: perplexity
research_sources:
  - >-
    https://www.bandsintown.com/e/107638307-the-targets-at-casa-lab-sp?came_from=209
network_extracted: true
entry_created: '2025-12-23T02:05:59.668547'
last_updated: '2025-12-23T08:16:06.158017'
artist_type: band
---

![](bruxos_modernos.jpg)

# Bruxos Modernos

## Quick Info
- **Genres**: Not specified
- **Spotify Popularity**: 0/100
- **Followers**: 91

## Biography
No comprehensive biographical information on the musical artist 'Bruxos Modernos' is available from credible sources. Search results reference unrelated artists and a single event mention without details on origins, career, style, or legacy.

*Enhanced with Perplexity AI research*

*Sources: [Source1](https://www.bandsintown.com/e/107638307-the-targets-at-casa-lab-sp?came_from=209)*

## Musical Connections

## Discography

### Top Tracks
1. Alguém Te Vê (Alguém Te Vê)
2. Essa Mentira (Não Cabe em Você) (Alguém Te Vê)
3. Bonita da Balada (Alguém Te Vê)
4. Linda Blair (Alguém Te Vê)
5. Cobras no Paraíso (Alguém Te Vê)
6. Makina Loka (Makina Loka)
7. Sister Mary (Makina Loka)
8. Ninguém é Dorian (Makina Loka)
9. Gagá (Makina Loka)

## External Links
- [Spotify](https://open.spotify.com/artist/4BqrcNbCVneZqkMWpMzGzP)
